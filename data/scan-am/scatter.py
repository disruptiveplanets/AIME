...
1e-2, 1e-5""".format(radius, LOW_ORDER[0], LOW_ORDER[1], LOW_ORDER[2], HIGH_ORDER[0], HIGH_ORDER[1],
    HIGH_ORDER[2], HIGH_ORDER[3], HIGH_ORDER[4], HIGH_ORDER[5], HIGH_ORDER[6])

for index, radius in enumerate(10**np.linspace(LOW_LOG_AM, HIGH_LOG_AM, NUM_DIVISIONS)):
    f = open("../../staged/{}-{:02}.txt".format(BASE_NAME, index), 'w')
    f.write(get_text(radius))
    f.close()
