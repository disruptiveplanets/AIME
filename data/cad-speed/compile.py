...
for plot_index in range(N_DIM):
    ...
    ax.plot(cadences, (param_data[1]-param_data[0]) * scale, color=f"C{i}", linewidth=1)
    ax.plot(cadences, (param_data[-1]-param_data[0]) * scale, color=f"C{i}", linewidth=1)
    ax.fill_between(cadences, (param_data[1]-param_data[0]) * scale, 
        (param_data[-1]-param_data[0]) * scale,  color=f"C{i}", alpha=0.3)

    ax.plot(cadences, (param_data[2]-param_data[0]) * scale, color=f"C{i}", linewidth=1)
    ax.plot(cadences, (param_data[-2]-param_data[0]) * scale, color=f"C{i}", linewidth=1)
    ax.fill_between(cadences, (param_data[2]-param_data[0]) * scale,
        (param_data[-2]-param_data[0]) * scale, color=f"C{i}", alpha=0.3)

    ax.plot(cadences, (param_data[3]-param_data[0]) * scale, color=f"C{i}", linewidth=1, linestyle='dashed')
    ...
    if plot_index >= 3:
        ax.set_ylabel(f"$\sigma({param_names[i]})$ ($\\times 10^{{-2}}$)", size=AXIS_SIZE)
    else:
        ax.set_ylabel(f"$\sigma({param_names[i]})$ ($\\times 10^{{-6}}$)", size=AXIS_SIZE)
