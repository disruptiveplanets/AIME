...
def get_text(perigee):
    perigee_m = perigee * 6_370_000
    semi_major_ax = MU / 6000**2
    eccentricity = 1 + perigee_m / semi_major_ax
    semi_latus_rectum = -semi_major_ax * (1 - eccentricity**2)
    cos_true_anomaly = (semi_latus_rectum / (10 * perigee_m) - 1) / eccentricity
    ...
    print(perigee, '\t', eccentricity, '\t', time_to_perigee / 3600)
...
1e-2, 1e-5""".format(perigee, gamma_0, LOW_ORDER[1], LOW_ORDER[2], HIGH_ORDER[0], HIGH_ORDER[1],
    HIGH_ORDER[2], HIGH_ORDER[3], HIGH_ORDER[4], HIGH_ORDER[5], HIGH_ORDER[6], high_lim, low_lim)

for ratio_index, perigee in enumerate(np.linspace(LOW_PERIGEE, HIGH_PERIGEE, NUM_DIVISIONS)):
