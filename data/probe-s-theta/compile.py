...
for plot_index in range(N_DIM):
    ...
    param_data = np.zeros(len(s_theta) * N_PERCENTILES).reshape(N_PERCENTILES, len(s_theta))
    for f in percentiles.keys():
        param_data[:,name_index[f]] = percentiles[f][plot_index]
    scale = 1e7 if plot_index < 3 else 1e2

    ax.plot(s_theta, (param_data[1]-param_data[0]) * scale, color=f"C{plot_index}", linewidth=1)
    ax.plot(s_theta, (param_data[-1]-param_data[0]) * scale, color=f"C{plot_index}", linewidth=1)
    ax.fill_between(s_theta, (param_data[1]-param_data[0]) * scale, 
        (param_data[-1]-param_data[0]) * scale,  color=f"C{plot_index}", alpha=0.3)

    ax.plot(s_theta, (param_data[2]-param_data[0]) * scale, color=f"C{plot_index}", linewidth=1)
    ax.plot(s_theta, (param_data[-2]-param_data[0]) * scale, color=f"C{plot_index}", linewidth=1)
...
plt.savefig("sigma-theta.pdf", bbox_inches="tight")
plt.savefig("sigma-theta.png", bbox_inches="tight")
