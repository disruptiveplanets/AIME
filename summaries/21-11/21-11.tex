...
\begin{figure}
  ...
  \caption{Ratio of 1- and 2-$\sigma$ uncertainties of the marginal PDFs to the observational uncertainty $\sigma_\theta$ as a function of the oblateness $\epsilon$ of the central planet. Linear best fit lines (black, dotted) are plotted over the 1- and 2-$\sigma$ uncertainties. Also shown are the oblatenesses of reference Solar System bodies \cite{pater_lissauer_2015}.}
...
The physical parameters $K_{\ell m}$ resulting from a fit to flyby data, while physical, do not completely describe an asteroid. Since the density distribution of the asteroid has many more degrees of freedom in general than the number of known $K_{\ell m}$, there are often many degenerate density distributions associated with one set of $K_{\ell m}$.

To avoid this degeneracy, we either (1) assume that the density is constant and solve for the asteroid surface (section \ref{sec:constant-density}), (2) assume that the surface of the asteroid is fixed and solve for the density distribution under various assumptions. (sections \ref{sec:laplace-rho}, \ref{sec:numerical-density}, and \ref{sec:like-density}). In the latter case, we can avoid the above degeneracy either by assuming $\nabla^2 \rho = 0$ (section \ref{sec:laplace-rho}), by using a numerical approach to find an average density distribution (section \ref{sec:numerical-density}), or by associating a likelihood with each density distribution and taking the distribution with maximum likelihood (section \ref{sec:like-density}).
...
\end{equation}
The integrand of each $K_{\ell m}$ therefore is proportional to $r(\theta, \phi)^{(3+\ell)}$ times constants and a spherical harmonic. We can write $r(\theta, \phi) = \sum_{\ell m} Y_{\ell m}^* C_{\ell m}$ without loss of generality, in which case equation \ref{eqn:constant-density-klm} becomes a polynomial of degree $3+\ell$ in terms of $C_{\ell' m'}$ and integrals of products of spherical harmonics. These integrals can be precomputed, since they are taken over the whole sphere $\theta \in (0, \pi)$ and $\phi \in (0, 2\pi)$. Equation \ref{eqn:constant-density-klm} can then be numerically solved to get $C_{\ell m}$. To remove the degeneracy caused by the factor of $\rho_m$, we instead solve given constraints of $K_{\ell m} / K_{00}$. (Note that $K_{00} = \rho_m / \mu_m$). We force the solution to reproduce the correct value of $a_m^2/K_{00}$, defined in equation \ref{eqn:am}. We calculate this via
\begin{equation}
  a_m^2 = \frac{\rho_m}{\mu_m}\int_\mathcal{A} d^3 r r^2 = \frac{K_{00}}{5}\oint_\mathcal{A} d^2 \bm r \cdot \bm r(r^2) = \frac{K_{00}}{5}\int_0^\pi d\theta \sin\theta \int_0^{2\pi}d\phi r(\theta, \phi)^5.
\end{equation}

Unfortunately, this method is extremely slow. As mentioned above, the polynomial to be solved is order $3+\ell$ with $L^2$ $C_{\ell m}$ components, where $L$ is the maximum value of $\ell$ considered. This makes the number of terms $(L^2)^{3+L} = O(L^L)$. It is therefore exponential in the level of $L$ fitted.
...
We present a final method to extract the density distribution of an asteroid with fixed surface and parameters $K_{\ell m}$. Suppose the asteroid is divided into $N$ finite, constant density units with volume $\Delta V$ (a square grid, for instance). We further write the density of the $i$th unit as $\mu_m / (N \Delta V)C_i$, where $C_i$ is a free real parameter. Approximating each unit as small, we may write 
\begin{equation}
  K_{\ell m} = \frac{1}{Na_m^\ell} \sum_i^N C_i R_{\ell m}(\bm r_i)
  \label{eqn:klm-finite-constraint}
\end{equation}
where $\bm r_i$ are the centers of the units. Equation \ref{eqn:klm-finite-constraint} is a matrix equation, which can be written in the form $\bm K = A \bm C$ where $K_{\ell m}$ have been arranged as a vector and $\bm C$ contains the $N$ values of $C_i$. The matrix $A$ is non-square and cannot be inverted to find $\bm C$ because equation \ref{eqn:klm-finite-constraint} has many solutions. However, by imposing a probability distribution on $\bm C$, we find one configuration of $\bm C$ that maximizes likelihood.

Suppose that the likelihood for $\bm C$ is multivariate-Gaussian distributed with no correlation, meaning that the log-likelihood is $\ln \mathcal{L} \propto -(x - \mu)^2$. We expect that $C_{i}$ average to one by the definition of $C_i$, so we set $\mu = 1$. Thus, the value of $\bm C$ we choose is the value which minimizes $|\bm C - \bm 1|^2$, where $\bm 1$ is an $N$-dimensional vector of only ones.

This problem has already been solved in the context of the Moore-Penrose pseudoinverse. First, we define $\bm k = \bm K - A\bm 1$, so that if $\bm k = A \bm c$, then $\bm C = \bm c + 1$ solves equation \ref{eqn:klm-finite-constraint}. We therefore attempt to minimize $|\bm c|^2$ to maximize the likelihood of density distributions.

The Moore-Penrose pseudoinverse of $A$ is defined as $A^+ = A^* (AA^*)^{-1}$, where $A^*$ is the adjoint of $A$. Ref. \cite{Barata2012} gives that all solutions to $\bm k = A \bm c$ are given by $\bm c = A^+\bm k + (\mathds{1}-A^+A)\bm z$ for any vector $\bm z$. Solving $\nabla_z c^2 = 0$ with this formula requires that $\bm z$ is an eigenvector of $\mathds{1}-A^+A$ with eigenvalue zero. The linearity of equation \ref{eqn:klm-finite-constraint} requires that, even if there are multiple zero-eigenvalue eigenvectors of $\mathds{1}-A^+A$, they must correspond to the same solution of $\bm c$. Thus, the density distribution of the asteroid is uniquely determined.
