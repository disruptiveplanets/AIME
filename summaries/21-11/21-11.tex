...
Figure \ref{fig:oblateness} shows no strong dependence of best-fit uncertainty on oblateness, as expected due to the weakness of the $J_{20}$ term in equation \ref{eqn:tidal-torque} compared to the dominant $J_{00}$ term. However, the fit parameter uncertainty does slightly decrease as oblateness increases on further study. \jtd{Give some quotes}. Other asteroid parameters $K_{3m}$ will likely be affected by oblateness to similar degrees. It is therefore reasonable to include that the presence of a large $J_{20}$ will not substantially affect the ability of this model to constrain asteroid interiors.
