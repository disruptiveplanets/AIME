...
\shorttitle{Constraining the Interiors of Asteroids Through Close Encounters}
...
\section{Density distributions, but redone}

In this section, we describe a pipeline to convert the density moments $K_{\ell m}$ to a density distribution $\rho(\bm r)$ which correctly reproduces the density moments via equation \ref{eqn:jlm}. We will also produce a function $\sigma_\rho(\bm r)$ which represents the uncertainty in density propagated from uncertainty in the $K_{\ell m}$ moments. Unfortunately, $\rho(\bm r)$ represents infinitely many degrees of freedom, whereas generally only a few $K_{\ell m}$ are known. Additional assumptions must therefore be made to produce one $\rho(\bm r)$ solution. One assumption we will make is that the surface of the asteroid has been accurately determined, from light curve data for instance. A method of computing a density distribution when no asteroid surface is known is presented in appendix \jtd{Appendix}. A few example sets of solutions are described below, but first we investigate the problem in a general formulation, agnostic of what assumptions beyond a known surface are chosen to eliminate the degeneracy.

\subsection{General formulation}
We assume that an indicator function $\mathds{1}(\bm r)$ has been determined such that $\mathds{1}(\bm r) = 1$ inside the asteroid and 0 outside, where $\mathds{1}(\bm r)$ is defined in some frame whose orientation with respect to Earth is known. The initial orientation of the asteroid's spin pole with respect to Earth is also known, which fixes the initial orientation of the local frame (the asteroid's frame) with respect to Earth up to one parameter $\gamma_0$, which is attained by a fit as seen in previous sections.

We define a new coordinate system, the ``hybrid frame,'' which coincides exactly with the initial orientation of the asteroid assuming that the fit result for $\gamma_0$ is perfectly accurate. The indicator function $\mathds{1}$ is then assumed to be known exactly in this frame, and the $K_{\ell m}$ components are known in the local frame which aligns with the hybrid frame up to uncertainty in $\gamma_0$. We will solve for $\rho(\bm r)$ in the hybrid frame given fit results $K_{\ell m}$ and the known $a_m$.

The density moments $K_{\ell m}$ defined in equation \ref{eqn:jlm} are not linear in $\rho(\bm r)$, but we may fix $a_m$ at the value assumed during the fit, removing the non-linearity induced by division by $a_m$. Furthermore, since the system is independent of the total asteroid mass, we set $\mu_m$ equal to an arbitrary constant which makes equation \ref{eqn:jlm} linear in $\rho(\bm r)$. Similarly, the form of equation \ref{eqn:am} guarantees that $a_m^2$ is linear in $\rho(\bm r)$. Equations \ref{eqn:am} and the $\ell, m = 0, 0$ component of equation \ref{eqn:jlm} can be used to enforce the choices of $\mu_m$ and $a_m^2$.

Suppose that we restrict the number of degrees of freedom of $\rho(\bm r)$ from infinity to $m$ by explicitly defining some function $\rho(\bm r, \bm D)$ for an $m$-dimensional vector $\bm D$ which controls the free parameters of $\rho$. We will leave the explicit definition of $\rho$ to the model descriptions below, but for now we assume that $\rho$ is linear in $\bm D$; i.e.,
\begin{equation}
  \rho(\bm r, \bm D) = \bm B(\bm r) \cdot \bm D
  \label{eqn:density-distro}
\end{equation}
for a $m$-dimensional vector $\bm B(\bm r)$. Thus, equations \ref{eqn:hlm-constraint} and \ref{eqn:b-constraint} represent a linear function that gives $K_{\ell m}$ in terms of $\bm D$. We further assume that the model describes a way to reverse this equation, to write
\begin{equation}
  \bm D = A \bm K.
  \label{eqn:density-model}
\end{equation}
Here, we have arranged $a_m^2$ and the components of $K_{\ell m}$ into a vector $K$, which we say has $n$ dimensions. The order of this arrangement is irrelevant, as long as it is kept consistent.

To calculate the uncertainty on $\bm D$ and therefore $\rho(\bm r)$, we first need the covariance matrix $\Sigma_K$ for $\bm K$. First, assume that the hybrid frame is offset from the local frame by some small angle $\Delta \gamma$, which results from uncertainty in $\gamma_0$. Then
\begin{equation}
...
\begin{equation}
  \rho(\bm r) = \rho(\bm r, A\bm K) \qquad \sigma^2_\rho(\bm r) = \bm B(\bm r)^T A \Sigma_K A^T \bm B(\bm r).
  \label{eqn:unc-rho}
\end{equation}
...
In order to restrict the space of density distributions that correspond to the known $H_{\ell m}$ so that only one distribution remains, we define a function $\mathcal{L}$ which takes large values for ``nice'' density distributions according to some metric of niceness. We then take the density distribution which maximizes $\mathcal{L}$. In this way, $\mathcal{L}$ can be thought of a likelihood for the density distribution, but it should not be confused with the likelihood used to extract the fit parameters $K_{\ell m}$. 

To employ this method, we divide the asteroid into a square grid of $m \gg n$ elements, each of which is assumed to have constant density $\rho_0$ plus some deviation $D_i$. This defines the model function $\bm B(\bm r)$, which is zeroed in all components except for the $i$th, where $i$ is the index of the grid element that contains $\bm r$.

The task is to define a likelihood $\mathcal{L}(\bm D)$ such that the minimum satisfies equation \ref{eqn:density-model}. This happens to be an uncorrelated Gaussian likelihood
\begin{equation}
...
To minimize this norm, we use the fact that equations \ref{eqn:jlm} and \ref{eqn:am} correspond exactly to a matrix equation $\bm K = M \bm D$, where the $i$th entry of every row of the $n\times m$ matrix $M$ is the integral presented in \ref{eqn:jlm} or \ref{eqn:am} evaluated over the $i$th finite element. Since $m>n$, we define the Moore-Penrose inverse of $M$ as
\begin{equation}
  M^+ = M^*(MM^*)^{-1}
  \label{eqn:mpi-underdetermined}
\end{equation}
where $M^*$ is the adjoint of $M$. The value of $\bm D$ that minimizes $|\bm D|^2$ and also satisfies equation \ref{eqn:density-model} is then $A=M^+$, which fully defines the model.

Note that this model is fast to compute; assuming fast matrix multiplication, the most time-consuming step is the matrix inversion of equation \ref{eqn:mpi-underdetermined}. However, $MM^*$ is an $n$-dimensional square matrix, which is very small compared to the number of finite elements $m$.


\subsection{Harmonic model}
We now explore a model that seeks to narrow down the space of allowed density distributions a different way than the likelihood model; we define the ``nice'' density distributions directly. They are the density distributions with zero Laplacian: $\nabla^2 \rho = 0$ inside the asteroid. The expansion of such a function in spherical coordinates is given by 
\begin{equation}
  \rho(\bm r) = \sum_{\ell, m} \frac{D_{\ell m}}{a_m^\ell} R_{\ell m}^*(\bm r)
  \label{eqn:harmonic-rho}
\end{equation}
where the terms which lead to $\rho \rightarrow \infty$ at the origin have been removed and $D_{\ell m}$ are free parameters. The complex conjugate was added for convenience. By setting a maximum on $\ell$, we restrict the number of degrees of freedom to $(\ell_\mathrm{max}+ 1)^2$. Choosing the same maximum $\ell$ as the maximum $\ell$ for the $H_{\ell m}$ moments, we have $m=n-1$ coefficients $D_{\ell m}$ which can be stacked into an $m$-dimensional vector $\bm D$.

Inserting equation \ref{eqn:harmonic-rho} into equations \ref{eqn:hlm-constraint} and \ref{eqn:b-constraint}, we get 
\begin{equation}
...
\end{equation}
This is an over-determined matrix equation $\bm K = M \bm D$, where $M$ is an $n \times m$ matrix. The Moore-Penrose inverse can therefore be used again to find an inverse $A=M^+$ which yields approximately correct $\bm D$ (approximate in that the norm of the error vector between $\bm K$ and $M \bm D$ is minimized). However, the form of the inverse changes due to the matrix now being taller than it is wide. The Moore-Penrose is now
\begin{equation}
  M^+ = (M^*M)^{-1} M^*
  \label{eqn:mpi-overdetermined}
\end{equation}
and the model concludes with $A=M^+$.

Note that, in the special case where the asteroid is a sphere of radius $R$, the matrix defined by equation \ref{eqn:harmonic-mat} is diagonal, with entries 
\begin{equation}
  M_{\ell m; \ell' m'} = \frac{4\pi R^3}{\mu_m} \frac{R^{2\ell}}{a_m^{2\ell}} \frac{\delta_{\ell \ell'} \delta_{m m'}}{(4\ell^2 + 8\ell + 3)(\ell - m)!(\ell+m)!}.
\end{equation}
A nonspherical perturbation will introduce small off-diagonal entries to $M$. This diagonal represents an interpretation of the physical meaning of $K_{\ell m}$; they are directly related through this form of $M$ and the matrix equation $\bm K = M \bm D$ to the coefficients of the spherical harmonic expansion of the asteroid in the spherical case.


\subsection{Lump model}
The above two simple models produce smooth density distributions that generally extend non-uniformity over large regions. In this section, we define a more complicated model which identifies discrete regions differing from the overall density of the asteroid to best fit the $K_{\ell m}$ fit results.

Suppose the asteroid contains $N$ `lumps' of constant mass $\mu_i$ displaced by distance $\bm x_i$ from the asteroid center of mass, and immersed in a constant density overall asteroid shape which is known in the form of $\mathds{1}(\bm r)$. For simplicity, we assume that all $N$ regions are ellipsoids with $d$ independent axis lengths. (For example, $d=3$ corresponds to an asymmetric ellipsoid, $d=2$ corresponds to a symmetric ellipsoid, and $d=1$ corresponds to a sphere.) Recall that $\mathds{1}(\bm r)$ is known in the hybrid frame where the origin is the center of mass of the asteroid. The displacement of the asteroid shape from the center of mass, which is the opposite of the net displacement of the discrete regions, is therefore observable from light curve analysis. The model therefore has $4N-3$ positional degrees of freedom and rotational degrees of freedom laid out in table \ref{tab:lump-dof}. These degrees of freedom should be compared to the known density moments, of which there is a square number $(L+1)^2$. However, three of these are the center of mass of the asteroid which is guaranteed, and one is the unconstrained total mass. So the total number of known parameters is $(L+1)^2 - 4$.
...
\begin{table}
  ...
  \begin{tabular}{cc|cccc}
    \hline \hline
        &  & \multicolumn{4}{c}{$N$} \\
        &  & 1  & 2  & 3  & 4  \\ \hline 
        & 1& \cellcolor{black}\color{white} 2 & \cellcolor{black}\color{white} 5 & \cellcolor{gray}\color{white} 8 & \cellcolor{gray}\color{white} 11 \\
    $d$ & 2& \cellcolor{black}\color{white} 5 & \cellcolor{gray}\color{white} 8 & \cellcolor{gray}\color{white} 11& 14 \\
        & 3& \cellcolor{gray}\color{white}  7 & \cellcolor{gray}\color{white} 10 & 13 & 16 \\
    \hline \hline
  \end{tabular}
  \caption{Total degrees of freedom $D$ as a function of $N$, the number of lumps modeled, and $d$, the number of independent moment of inertia axes considered for each lump. The configurations with $D \leq$ the five known parameters not including $K_{3m}$ are colored black, and those including seven $K_{3m}$ parameters are colored gray.}
  \label{tab:lump-dof}
\end{table}
...
\begin{equation}
  ...
  \label{eqn:klm-stack}
\end{equation}
where $K_{\ell m}^i$ and $a_i^2$ obey the same definition as $K_{\ell m}$ in equation \ref{eqn:jlm} and $a_m$ in equation \ref{eqn:am} respectively. Crucially, note that the normalizing factors $1/(\mu_m a_m^\ell)$ and $1/\mu_m$ in equations \ref{eqn:jlm} and \ref{eqn:am} do not change to their counterparts for each lump; they remain the known asteroid-wide values. The zero-indexed parameters indicate the density moments of the asteroid medium surrounding the lumps.

The prime in equation \ref{eqn:klm-stack} denotes that the moments are calculated in the hybrid frame. We can relate the primed moments to the moments calculated relative to each body's center of mass via the translation rules for solid spherical harmonics:
\begin{equation}
  K_{\ell m}'^i = \sum_{\ell' m'} (-1)^{\ell - \ell'} R_{\ell - \ell', m - m'}(\bm x_i) K_{\ell' m'}^i \qquad a_i'^2 = a_i^2 + x_i^2 K_{00}
  \label{eqn:translate-klm}
\end{equation}
The dummy indices $\ell', m'$ should only be summed over values in which $\ell-\ell' \geq 0$ and $|m-m'| \leq \ell - \ell'$. Here, $K_{00}^i$ is the added mass $\mu_i$ of lump $i$, while $K_{1m}=0$ and $K_{2m}$ incorporate its orientation and moment of inertia ratios. The volume is constrained by $a_i^2$. These values map directly onto an ellipsoid representation via equations \ref{eqn:ellipsoid-klm}, so that if $K_{\ell m}^i$, $a_i^2$, $\mu_i$, and $x_i$ are known, then the density distribution is known. Note that if $\bm x_i$ is known, then equation \ref{eqn:translate-klm} are linear in $K_{\ell m}^i$. Therefore, if we define $\bm D$ to contain $K_{2m}^i$ and $a_i^2$ in some order, then equations \ref{eqn:translate-klm} and \ref{eqn:klm-stack} define a matrix equation $\bm K = \bm C + M \bm D$ which can be solved by setting $A$ equal to the Moore-Penrose inverse of $M$. Henceforth, we assume that the dimensions of $\bm D$ are less than or equal to the number of known parameters (the shaded regions of table \ref{tab:lump-dof}), so that $A$ is given by equation \ref{eqn:mpi-overdetermined}). Note that the $\bm C$ term of this equation came from the expansion of $K_{\ell m}'^0$ and $a_0^2$, which is written in terms of the already-known displacement of the asteroid surface from its center of mass.

This model's strategy will be to choose $\bm x_i$ and $\mu_i$ via a nonlinear process, then fit for the values $K_{\ell m}^i$ and $a_i^2$ using the linear format described in the introduction to this section. The constraints on $\bm x_i$ and $|mu_i$ are primarily
\begin{equation}
