...
int Asteroid::simulate(double cadence, std::vector<double>& resolved_data) {
    ...
    const double expire_distance_sqr = distance_ratio_cut * distance_ratio_cut * pericenter_pos * pericenter_pos;
    ...
    for (time = MIN_DT-expire_time; time+MIN_DT < expire_time; time += dt) {
        ...
        while (int(time / cadence) > cadence_index) {
            resolved_data.push_back(quat.r());
            resolved_data.push_back(quat.i());
            resolved_data.push_back(quat.j());
            resolved_data.push_back(quat.k());
            cadence_index++;
        }
        frames++;
    }
