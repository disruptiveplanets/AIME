...
UNCERTAINTY_MODEL = random_vector.UNIFORM_ANGLE
...
def fit_function(theta, target_length=None):
    ...
    if target_length is not None:
        while len(resolved_data)//4 < target_length:
            resolved_data.append(resolved_data[-4])
            resolved_data.append(resolved_data[-4])
            resolved_data.append(resolved_data[-4])
            resolved_data.append(resolved_data[-4])
    return quaternion.as_quat_array(resolved_data)
...
def log_probability(theta, y):
    prior = log_prior(theta)
    like = log_likelihood(theta, y)
...
def get_data_cut(drc):
    ...
    if ASTEROIDS_MAX_J == 0:
        return len(asteroids_0_2.simulate(cadence, jlms, theta_true, radius,
            ...
            drc, enforce_drc, VELOCITY_MUL)) // 4
    elif ASTEROIDS_MAX_J == 2:
...
def minimize_function(theta, simulate_func, l_index, cut_index):
    ...
    resolved_data = quaternion.as_quat_array(resolved_data) # Already normalized
    
    want_length = data_cuts[l_index][cut_index]
    if len(resolved_data) != want_length:
        resolved_data = list(resolved_data)
        while len(resolved_data) < want_length:
            resolved_data.append(resolved_data[-1])
        while len(resolved_data) > want_length:
            del resolved_data[-1]
...
def get_minimum(arg):
    y, point, fix_theta, l, bounds, true_redchi = arg
    ...
    for cut_index in range(len(DISTANCE_RATIO_CUTS)):
        ...
        start_redchi = 2 *  minimize_log_prob_cut(point) / data_cuts[l-2][cut_index]
        ...
        if not bfgs_min.success:
            logging.warning(f"One of the minimum finding points failed at l {l} and cut index {cut_index} and redchi {bfgs_min.fun / data_cuts[l-2][cut_index]}.")
            logging.error(bfgs_min)
            return None, None, None, None

        end_redchi = 2 * bfgs_min.fun / data_cuts[l-2][cut_index]
    ...
    return 2 * minimizing_likelihood / len(y), result, new_evals, new_evecs
...
def minimize(l, fix_theta):
    ...
    while len(parameter_points) < NUM_MINIMIZE_POINTS_PER * len(theta_true):
        ...
        try:
            ...
            continue
        parameter_points.append([y, point, fix_theta, l, bounds, true_redchi])
...
def populate(evals, diagonalizer, count, start):
    ...
    if not np.all(np.isfinite(spacing)):
        ...
        spacing[~np.isfinite(spacing)] = MAX_SPACING[~np.isfinite(spacing)]
    if np.any(spacing > 1):
    ...
    for i, point in enumerate(global_points):
        while True:
            logprob = log_probability(point, y)
...
def mcmc_fit(theta_start, evals, evecs, index):
    ...
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(N_WALKERS, N_DIM, log_probability,
            args=(y,), backend=backend, pool=pool)
...
if __name__ == "__main__":
    ...
    assert(int(max_j) == ASTEROIDS_MAX_J)
    assert(int(max_k) == ASTEROIDS_MAX_K)
    ...
    true_redchi = 2 * minimize_log_prob(y, theta_true, [], real_sim_func, 0, -1) / len(y)
