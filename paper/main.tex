...
Figure \ref{fig:den-uniform} displays density distributions and uncertainties extracted by both the finite element and lumpy models for the reference asteroids. They are consistent with the moments extracted by the MCMC in section \ref{sec:fit}, even for non-ellipsoidal asteroid shapes (not shown).
...
%% Working on cutting down the uncertainties in the large-uncertainty regions
...
The distributions of figure \ref{fig:den-move} reproduce the extracted density moments; nevertheless the distributions extracted by the finite model match the true distributions less well. The deviation from the true density extends to as much as 17\% in some locations, leading to a maximum significance of $0.52\sigma$. Visually the finite element model has spread the high-density core into the rest of the asteroid, but this deviation from the true density distribution does not have high significance due to the similarly large model-driven uncertainty of the finite element model.

%% Julien wants a side by side figure to display how well the core is extracted

%% Julien also mentioned adding a method to detect deviations.
...
In the previous section, we saw that the uncertainty in density over the asteroid is strongly model-dependent. In section \ref{sec:density-compare} below, we further study the model outputs by considering two more sample asteroid extractions.
...
We will consider two example scenarios which lead to inflated density distribution uncertainties independent of the actual asteroid properties or the data quality. Specifically, we consider the lumpy model's degeneracy for a centred core and the two-core lumpy model. This highlights the model dependence of the density distributions extracted from density moments.
...
%% Why do we have two thresholds?

Figure \ref{fig:net-uncertainty} depicts $\langle \sigma_\rho / \rho \rangle$ as a function of these other encounter properties, except for the dependence of $\langle \sigma_\rho / \rho \rangle$ on initial spin direction which is better shown as a map in figure \ref{fig:spin-uncertainty}. We define two density distribution uncertainty thresholds termed the ``weak'' and ``strong'' thresholds. These values are $\langle \sigma_\rho / \rho \rangle = 10^{-4}$ (weak) and $10^{-3}$ (strong), for density distributions extracted via the lumpy model. They are manually chosen, and adjustments to them or to the lumpy model will affect the data of table \ref{tab:threshold-summary}. They are shown as red lines in figures \ref{fig:net-uncertainty} and \ref{fig:spin-uncertainty}, and the encounter properties which exceed these thresholds are listed in table \ref{tab:threshold-summary} for reference. 

The thresholds are applied to the lumpy model distribution because the large, model-induced uncertainty of the finite element model makes its density distributions only slightly dependent on encounter parameters. This is a further example of model-induced uncertainty in density distributions.
