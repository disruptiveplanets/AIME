...
The lumpy model produces zero-mass lumps in this case, and produces very small $\Delta K_{\ell m}$. \jtd{What are the exact parameters of the lump? Look at log files}.
...
\jtd{Why do the densities go negative?}

\jtd{What's wrong with the lumpy model? Wasn't it giving really good results before?}
...
\begin{enumerate}
  \item An orbit around Earth with $6$ km s$^{-1}$ excess velocity and perigee at 5 Earth radii. This orbit was chosen to roughly match that of \jtd{Number} Apophi s\jtd{Cite Apophis close approach}
...
\jtd{Pull the correct plots from supercloud.}
