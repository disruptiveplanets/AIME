...
It will be useful to have a rule-of-thumb ``posterior uncertainty threshold,'' above which we say that posterior uncertainty $\sigma(K_{\ell m})$ is too large to yield meaningful estimates of density distribution. There are many possible definitions of posterior uncertainty, but we explore one in this paper which is outlined in appendix \ref{app:sigma-threshold}. This leads to cut-offs of the encounter properties which lead to posterior uncertainty less than the threshold. These critical values are shown in table \ref{tab:threshold-summary} as a summary, and are discussed in detail in the following sections.

\jtd{This Klm uncertainty threshold language is still in the paper, although the numbers in the above table reflect the method I used for the conference, which is asking what encounter properties yield density distributions with uncertainty 100 or 20\% of the local density. The caveat to the latter, density-distribution based, uncertainty threshold is that it changes based on which models you use and how many Klms you choose to fit due to the fact that the density distribution is underdetermined. A constraint on the density moments is more general, but harder to define. My feeling is that we should use this distribution-based constraint, and mention the caveats here. I'll write up all this if Julien agrees.}

\begin{table}
  \centering
  \begin{tabular}{lll} \hline
    Encounter property & $\sigma_\rho / \rho = 100\%$ & $\sigma_\rho / \rho = 20\%$ \\ \hline
  ...
  \end{tabular}
  \caption{For each encounter property discussed in section \ref{sec:discussion}, the cut-offs (i.e., the values which leads to $\sigma(K_{3 m})$ at the threshold value) are given. Properties inside the strong cut-off have all $\sigma(K_{3m})$ under the threshold value, while properties inside the weak cut-off have at least one $\sigma(K_{3m})$ under the threshold value. The reference asymmetric asteroid is used. \jtd{summary}}
  ...
  \begin{landscape}
    \begin{figure}
      \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=0.33\linewidth]{figs/in-likelihood.pdf}\hfill
        \includegraphics[width=0.33\linewidth]{figs/in-harmonic.pdf}\hfill
        \includegraphics[width=0.33\linewidth]{figs/in-lumpy.pdf}
