...
\begin{table}
  ...
  \begin{tabular}{lll} \hline
    ...
    Perigee ($r_p$) & & \\
    Spin pole uncertainty ($\sigma_\theta$) & & \\
    Spin period uncertainty ($P_\omega \sigma_\rho$) & & \\
    Asteroid length ($a_\mathcal{A}$) & & \\
    Cadence ($\Delta t$) & & \\
    Data gap ($T_\text{gap}$) & & \\
    Rotational period ($P_\omega$) & & \\ \hline
  \end{tabular}
  \caption{For each encounter property discussed in section \ref{sec:discussion}, the cut-offs (i.e., the values which leads to $\sigma(K_{3 m})$ at the threshold value) are given. Properties inside the strong cut-off have all $\sigma(K_{3m})$ under the threshold value, while properties inside the weak cutoff have at least one $\sigma(K_{3m})$ under the threshold value. The reference asymmetric asteroid is used. \jtd{summary}}
...
\begin{figure*}
  ...
  \includegraphics[width=0.33\textwidth]{figs/asym-sph-unc-likelihood.pdf}\hfill
  \includegraphics[width=0.33\textwidth]{figs/asym-sph-unc-harmonic.pdf}\hfill
...
\jtd{The failure of this model (which is not yet reflected in the text well) makes me think we really should only do the finite element analysis. Though it is slow.}
