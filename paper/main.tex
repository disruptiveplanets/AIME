...
\begin{table}
  ...
  \begin{tabular}{lll} \hline
    ...
    \hline
  \end{tabular}
  \caption{The weak and strong thresholds on physical / observational properties (\textit{top} / \textit{bottom}) consistent with useable density distribution uncertainty. Perigee and observational uncertainty are the most constraining properties, followed by asteroid length, rotational period, and observational cadence.}
...
\begin{figure*}
  ...
  \caption{Median ratio of density uncertainty to density computed over the reference asteroid density distribution as a function of various encounter properties. Density distributions from both the lumpy and finite element models are used. The benchmark uncertainty thresholds (\textit{red lines}) are shown and the properties that exceed these thresholds are shaded. The figures illustrate the marked difference in density distributions that results from using different models.}
...
In table \ref{tab:threshold-summary}, we present the property ``thresholds'' required in order to obtain meaningful information about the asteroid density distribution. These are collected by evaluating the median uncertainty ratio of the asteroid density distribution extracted by the lumpy model as a function of each encounter property and identifying when it exceeds $\sigma_\rho / \rho = 10^{-4}$ (the weak threshold) and $\sigma_\rho / \rho = 10^{-3}$ (the strong threshold). These thresholds are also shown in figures \ref{fig:scan-physical} to \ref{fig:scan-observational} as vertical red lines. The values of $\sigma_\rho / \rho$ are plotted as a function of encounter properties in figure \ref{fig:net-uncertainty}, similarly to figures \ref{fig:scan-physical} and \ref{fig:scan-observational}.
