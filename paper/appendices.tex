...
We will use the same tools to extract data from the orientation data set as we did in the main text for the angular velocity data set. The only item that requires redefinition is the likelihood used by the MCMC (equation \ref{eqn:log-likelihood}), which depends on a model for the uncertainty of orientation observations.
...
\begin{figure}
  \centering
  % \includegraphics{figs/observational-moment-uncertainty}
  \caption{\jtd{Write}}
