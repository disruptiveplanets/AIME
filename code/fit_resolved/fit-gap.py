...
def fit_function(theta, target_length=None):
    ...
    if target_length is not None:
        while len(resolved_data)//3 < target_length:
            ...
            resolved_data.append(resolved_data[-3])
            resolved_data.append(resolved_data[-3])
    return np.asarray(resolved_data).reshape(-1, 3)[uncut_mask]
...
times = np.arange(data_cuts[-1][-1]) * cadence / 3600
times -= times[-1] / 2

# Generate mask
uncut_mask = abs(times) > gap
chop_index = 0
...
for line in data_cuts:
    ...
    len_line = []
    for entry in line:
        new_mask = uncut_mask[:entry]
        cut_line.append(new_mask)
        len_line.append(np.sum(new_mask))
    cut_masks.append(cut_line)
    len_data_cuts.append(len_line)
...
plt.xlim(-8.5, 8.5)
for tier in len_data_cuts:
    for drc in tier:
        plt.axvline(x=x_display[drc-1], color='k')
plt.legend()
plt.show()
...
def minimize_function(theta, simulate_func, l_index, cut_index):
    ...
    return np.asarray(resolved_data).reshape(-1, 3)[cut_masks[l_index][cut_index]]
...
def minimize_log_prob(float_theta, fix_theta, simulate_func, l_index, cut_index):
    ...
    # Return chisq
    chisq = 0
    want_length = len_data_cuts[l_index][cut_index]
...
def get_minimum(arg):
    ...
    for cut_index in range(len(DISTANCE_RATIO_CUTS)):
        ...
        start_redchi = 2 *  minimize_log_prob_cut(point) / len_data_cuts[l-2][cut_index] / 3
        ...
        if not bfgs_min.success:
            logging.warning(f"One of the minimum finding points failed at l {l} and cut index {cut_index} and redchi {bfgs_min.fun / len_data_cuts[l-2][cut_index] / 3}.")
            logging.error(bfgs_min)
            return None, None, None, None

        end_redchi = 2 * bfgs_min.fun / len_data_cuts[l-2][cut_index] / 3
