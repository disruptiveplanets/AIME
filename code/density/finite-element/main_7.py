...
def mcmc_fit(theta_start, output_name, info, generate=True):
    if generate:
        ...
        with Pool() as pool:
            sampler = emcee.EnsembleSampler(N_WALKERS, N_FREE, log_probability, args = (info,), backend=backend, pool=pool)
        ...
        sampler = emcee.EnsembleSampler(N_WALKERS, N_FREE, log_probability, args=(info,), backend=backend)
...
def min_func_mcmc(seed, info, result):
    ...
    while not result.is_set():
        val = [(local_rng.random() * 4 + 0.5) * info.mean_density for _ in range(N_FREE)]
        try:
...
def get_stats_from_long_samples(long_samples):
    short_samples = long_samples[:, :N_FREE]
...
def min_func_ls_sq(params, result):
    if result.is_set():
        raise Exception()
    thetas = [params[f"d{i}"].value for i in range(N_FREE)]
...
def get_single_density(seed, info, result):
    ...
    local_rng.seed(seed)
    while not result.is_set():
        val = [local_rng.random() * 4 * info.mean_density for _ in range(N_FREE)]
...
def load(name, asteroid, surface_am, sample_path, division, generate=True):
    ...
    # Set order
    rlm_mat = np.zeros((N_FITTED_MOMENTS + N_CONSTRAINED, N_ALL))
    # Unconstrained
    ...
    rlm_fixed_inv = inv(rlm_mat[np.arange(N_FREE, N_ALL),:][:,np.arange(N_FREE, N_ALL)])
    rlm_cross = rlm_mat[np.arange(N_FREE, N_ALL),:][:,np.arange(0, N_FREE)]
