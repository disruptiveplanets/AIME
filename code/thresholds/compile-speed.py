...
with open("cad-speed-contour.npy", 'rb') as f:
   uncs = np.load(f)[:,:,2] # Use median

# Get percentiles
with open("../../data/cad-speed-contour/percentiles.dat", 'r') as f:
...
for name in percentiles.keys():
    dir_name = name[:12]
    with open(f"../../data/cad-speed-contour/{dir_name}/{dir_name}.txt", 'r') as f:
...
cbar = plt.colorbar(p)
plt.ylabel(f"$v_\\mathrm{{orbit}} / v_\\mathrm{{physical}}$")
plt.xlabel(f"$\Delta t$ (min)")
plt.savefig("cad-speed-contour-net.pdf", bbox_inches="tight")
plt.savefig("cad-speed-contour-net.png", bbox_inches="tight")
